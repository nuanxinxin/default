...
//        return $this->_returnMsg(500, '注册通道已关闭！');
    ...
    public function creditMoneyCollection()
    {
        try {
            ...
            if ($user->credit_money < Setting::getConfigValue(4, 'withdrawal_condition')) {
                abort(500, '信用币余额不足' . Setting::getConfigValue(4, 'withdrawal_condition') . '元');
            }
            $model = new CreditRecord;
...
//                ),
    ...
    {
        file_put_contents('./pay_notify.txt', json_encode($this->request->param()));
        $Pay = new Pay;
        $orderId = $Pay->Notify($this->request->param());
        if ($orderId !== false) {
