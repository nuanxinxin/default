<?php 
namespace org\allinpayInter;
use org\allinpayInter\ArrayXml;
use think\exception;
class PhpTools{
	 
	
	const certFile = __DIR__ . DIRECTORY_SEPARATOR . "key/allinpay-pds.pem";//通联公钥证书
	
	
	//const privateKeyFile =  __DIR__ . DIRECTORY_SEPARATOR .'key/20060400000044502.pem';//商户私钥证书
	
	
	
	const privateKeyFile =  __DIR__ . DIRECTORY_SEPARATOR .'key/20042100000837904.p12';
...
//	const  apiUrl = 'http://172.16.1.11:8080/aipg/ProcessServlet';//通联系统对接请求地址（内网）
 ...
 //   const apiUrl = 'http://113.108.182.3:8083/aipg/ProcessServlet';//通联系统对接请求地址（外网,商户测试时使用）
	...
	public function verifyXml($xmlResponse){	
		...
		$result=file_get_contents(PhpTools::certFile);
		...
		if ($flag) {
			...
			throw Exception ('Verified:Failed.');
		    return FALSE;
		}
	}
...
//		echo ($xmlSignSrc);
		...
		$pKeyId=openssl_pkcs12_read($privateKey,$certs,PhpTools::password);
		openssl_sign ( $xmlSignSrc, $signature, $certs['pkey']);
...
//		openssl_sign($xmlSignSrc, $signature, $pKeyId);
	...
	public function send($params){
		...
		if (! isset($response['body'])) {
			throw Exception ('Error: HTTPS REQUEST Bad.');
		}
		//获取返回报文
